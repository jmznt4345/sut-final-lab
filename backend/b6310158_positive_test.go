...
type Customer struct {
	...
	Email      string
	CustomerID string `valid:"matches(L\\b{7}$)~กรอก CustomerID ขึ้นต้นด้วย L หรือ M หรือ H ตามด้วยเลข7หลัก"`
}
...
func testCutomerID(t *testing.T) {
	...
	g.Expect(ok).NotTo(gomega.BeTrue())
	g.Expect(err).ToNot(gomega.BeNil())
	g.Expect(err.Error()).To(gomega.Equal("กรอก CustomerID ขึ้นต้นด้วย L หรือ M หรือ H ตามด้วยเลข7หลัก"))
}
